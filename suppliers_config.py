...
the optimisation needs:
    ...
    capacity       max units suppliable in the planning period
...
SUPPLIERS: dict[str, dict] = {
    "S01": {"capacity": 1200, "unit_cost":  8.00, "holding_cost": 0.90, "defect_rate": 0.040, "delivery_time": 14, "min_order": 100},
    "S02": {"capacity":  900, "unit_cost":  8.50, "holding_cost": 0.85, "defect_rate": 0.030, "delivery_time": 12, "min_order":  80},
    "S03": {"capacity":  500, "unit_cost":  9.00, "holding_cost": 0.70, "defect_rate": 0.022, "delivery_time":  9, "min_order":  50},
    "S04": {"capacity":  650, "unit_cost":  9.50, "holding_cost": 0.75, "defect_rate": 0.018, "delivery_time": 10, "min_order":  60},
    "S05": {"capacity":  400, "unit_cost": 10.00, "holding_cost": 0.60, "defect_rate": 0.015, "delivery_time":  7, "min_order":  50},
    "S06": {"capacity":  350, "unit_cost": 10.50, "holding_cost": 0.55, "defect_rate": 0.012, "delivery_time":  6, "min_order":  40},
    "S07": {"capacity":  300, "unit_cost": 11.50, "holding_cost": 0.50, "defect_rate": 0.008, "delivery_time":  5, "min_order":  40},
    "S08": {"capacity":  550, "unit_cost":  9.25, "holding_cost": 0.72, "defect_rate": 0.020, "delivery_time": 11, "min_order":  60},
    "S09": {"capacity":  800, "unit_cost":  8.75, "holding_cost": 0.80, "defect_rate": 0.028, "delivery_time": 13, "min_order":  80},
    "S10": {"capacity":  250, "unit_cost": 12.00, "holding_cost": 0.48, "defect_rate": 0.006, "delivery_time":  4, "min_order":  30},
}

# Total pool capacity, useful for sanity-checking forecasted demand against
# what the supplier base can actually absorb.
...
def dea_arrays(suppliers: dict = SUPPLIERS):
    ...
    INPUTS to minimise : unit cost, holding cost, delivery time, defect rate.
    OUTPUT to maximise  : capacity (the throughput a supplier can offer).
    Mirrors `data.dea_arrays` so either catalogue can feed `dea.py`.
    """
    inputs = {
        j: [s["unit_cost"], s["holding_cost"], s["delivery_time"], s["defect_rate"]]
        for j, s in suppliers.items()
    }
    outputs = {j: [s["capacity"]] for j, s in suppliers.items()}
...
if __name__ == "__main__":
    ...
    hdr = f"{'id':<4} {'cap':>5} {'cost':>6} {'hold':>5} {'defect':>7} {'lead':>5} {'minQ':>5}"
    print(hdr)
    print("-" * len(hdr))
    for j, s in SUPPLIERS.items():
        print(f"{j:<4} {s['capacity']:>5} {s['unit_cost']:>6.2f} "
              f"{s['holding_cost']:>5.2f} {s['defect_rate']*100:>6.1f}% "
              f"{s['delivery_time']:>4}d {s['min_order']:>5}")
